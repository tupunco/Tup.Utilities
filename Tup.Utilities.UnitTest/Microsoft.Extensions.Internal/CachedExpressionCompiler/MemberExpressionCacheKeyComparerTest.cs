...
namespace Microsoft.Extensions.Internal.Tests
{
    ...
    public class MemberExpressionCacheKeyComparerTest
    {
        ...
        {
            Assert2.Equal(key1, key2, Comparer);

            var hashCode1 = Comparer.GetHashCode(key1);
            var hashCode2 = Comparer.GetHashCode(key2);
            Assert2.Equal(hashCode1, hashCode2);

            var cachedKey1 = key1.MakeCacheable();

            Assert2.Equal(key1, cachedKey1, Comparer);
            Assert2.Equal(cachedKey1, key1, Comparer);

            var cachedKeyHashCode1 = Comparer.GetHashCode(cachedKey1);
            Assert2.Equal(hashCode1, cachedKeyHashCode1);
        }
        ...
        {
            var memberExpression = Assert2.IsAssignableFrom<MemberExpression>(expression.Body);
