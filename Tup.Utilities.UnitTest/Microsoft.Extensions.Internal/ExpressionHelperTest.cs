...
namespace Microsoft.Extensions.Internal.Tests
{
    ...
    public class ExpressionHelperTest
    {
        ...
        #region GetExpressionText_ReturnsExpectedExpressionText

        [TestMethod]
        public void GetExpressionText_ReturnsExpectedExpressionText()
        {
            foreach (var item in ExpressionAndTexts)
            {
                GetExpressionText_ReturnsExpectedExpressionText_Theory((LambdaExpression)item.Key, item.Value);
            }
        }
        ...
        #region GetExpressionText_CachesExpression

        [TestMethod]
        public void GetExpressionText_CachesExpression()
        {
            foreach (var item in CachedExpressions)
            {
                GetExpressionText_CachesExpression_Theory((LambdaExpression)item);
            }
        }
        ...
        #region GetExpressionText_DoesNotCacheIndexerOrUnsupportedExpression

        [TestMethod]
        public void GetExpressionText_DoesNotCacheIndexerOrUnsupportedExpression()
        {
            foreach (var item in IndexerExpressions)
            {
                GetExpressionText_DoesNotCacheIndexerOrUnsupportedExpression_Theory((LambdaExpression)item);
            }

            foreach (var item in UnsupportedExpressions)
            {
                GetExpressionText_DoesNotCacheIndexerOrUnsupportedExpression_Theory((LambdaExpression)item);
            }
        }
        ...
        #region GetExpressionText_CacheEquivalentExpressions

        [TestMethod]
        public void GetExpressionText_CacheEquivalentExpressions()
        {
            foreach (var item in EquivalentExpressions)
            {
                GetExpressionText_CacheEquivalentExpressionsn_Theory((LambdaExpression)item.Key, (LambdaExpression)item.Value);
            }
        }
        ...
        #region GetExpressionText_CheckNonEquivalentExpressions

        [TestMethod]
        public void GetExpressionText_CheckNonEquivalentExpressions()
        {
            foreach (var item in NonEquivalentExpressions)
            {
                GetExpressionText_CheckNonEquivalentExpressions_Theory((LambdaExpression)item.Key, (LambdaExpression)item.Value);
            }
        }
        ...
        public void GetExpressionText_WithinALoop_ReturnsExpectedText()
        {
            ...
            for (var i = 0; i < 2; i++)
            {
                ...
                // Assert i
                Assert2.Equal(expectedText, result);
            }
        }
